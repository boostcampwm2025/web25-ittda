import {
  ArgumentsHost,
  BadRequestException,
  HttpException,
  HttpStatus,
} from '@nestjs/common';
import { AllHttpExceptionFilter } from './AllHttpExceptionFilter';

describe('AllHttpExceptionFilter', () => {
  const createHost = (requestId?: string) => {
    let responseBody: unknown;
    const response = {
      headersSent: false,
      setHeader: jest.fn(),
      status: jest.fn().mockReturnThis(),
      json: jest.fn((body: unknown) => {
        responseBody = body;
      }),
    };
    const request = {
      headers: requestId ? { 'x-request-id': requestId } : {},
    };
    const host = {
      switchToHttp: () => ({
        getRequest: () => request,
        getResponse: () => response,
      }),
    } as ArgumentsHost;

    return {
      host,
      response,
      getResponseBody: () => responseBody,
    };
  };

  it('알 수 없는 예외의 원문을 숨긴다', () => {
    const filter = new AllHttpExceptionFilter();
    const { host, response } = createHost('req_test');

    filter.catch(new Error('password=secret'), host);

    expect(response.status).toHaveBeenCalledWith(
      HttpStatus.INTERNAL_SERVER_ERROR,
    );
    expect(response.json).toHaveBeenCalledWith({
      success: false,
      data: null,
      error: {
        code: 'INTERNAL_SERVER_ERROR',
        message: '일시적인 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.',
        requestId: 'req_test',
      },
    });
  });

  it('명시적인 코드가 있는 4xx 예외의 메시지만 노출한다', () => {
    const filter = new AllHttpExceptionFilter();
    const { host, response } = createHost('req_business');
    const exception = new HttpException(
      {
        code: 'ALREADY_JOINED',
        message: '이미 참여한 그룹입니다.',
        details: { constraint: 'group_member_unique' },
      },
      HttpStatus.CONFLICT,
    );

    filter.catch(exception, host);

    expect(response.json).toHaveBeenCalledWith({
      success: false,
      data: null,
      error: {
        code: 'ALREADY_JOINED',
        message: '이미 참여한 그룹입니다.',
        requestId: 'req_business',
      },
    });
  });

  it('DTO 검증 오류를 공통 검증 오류로 치환한다', () => {
    const filter = new AllHttpExceptionFilter();
    const { host, response } = createHost('req_validation');
    const exception = new BadRequestException([
      'email must be an email',
      'password must be longer than 8 characters',
    ]);

    filter.catch(exception, host);

    expect(response.json).toHaveBeenCalledWith({
      success: false,
      data: null,
      error: {
        code: 'VALIDATION_ERROR',
        message: '입력값을 확인해 주세요.',
        requestId: 'req_validation',
      },
    });
  });

  it('요청 ID가 없으면 새 요청 ID를 생성한다', () => {
    const filter = new AllHttpExceptionFilter();
    const { host, response, getResponseBody } = createHost();

    filter.catch(new BadRequestException('raw message'), host);

    const body = getResponseBody() as {
      error: { requestId: string; message: string };
    };
    expect(body.error.requestId).toMatch(/^req_[0-9a-f-]{36}$/);
    expect(body.error.message).toBe('잘못된 요청입니다.');
    expect(response.setHeader).toHaveBeenCalledWith(
      'X-Request-Id',
      body.error.requestId,
    );
  });

  it('안전하지 않은 요청 ID는 새 요청 ID로 교체한다', () => {
    const filter = new AllHttpExceptionFilter();
    const { host, getResponseBody } = createHost('req_valid\nforged-log');

    filter.catch(new BadRequestException(), host);

    const body = getResponseBody() as {
      error: { requestId: string };
    };
    expect(body.error.requestId).toMatch(/^req_[0-9a-f-]{36}$/);
  });
});
