...
describe('FeedGroupQueryService.getPastFeedForGroup', () => {
  ...
  it('그룹 스코프로만 필터링하고, 커서가 없으면 최신순으로 limit만큼 조회한다', async () => {
    const qb = buildService([]);

    await service.getPastFeedForGroup('group-1', 'user-1', undefined, 5);

    expect(qb.where).toHaveBeenCalledWith('p.scope = :scope', {
      scope: 'GROUP',
    });
    expect(qb.andWhere).toHaveBeenCalledWith('p.groupId = :groupId', {
      groupId: 'group-1',
    });
