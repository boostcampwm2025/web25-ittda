...
export class AddUserGuestGroupEntities1768589247868 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."uq_post_media_block_sort"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."uq_post_media_thumbnail_per_post"`,
    );
    await queryRunner.query(
      `CREATE TABLE IF NOT EXISTS "guest_sessions" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "expiresAt" TIMESTAMP NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), CONSTRAINT "PK_7c077389b040139ae9487c1b03e" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `DO $$ BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_type WHERE typname = 'group_invites_permission_enum'
        ) THEN
    ...
    await queryRunner.query(
      `DO $$ BEGIN
        IF EXISTS (
          SELECT 1 FROM pg_type WHERE typname = 'post_blocks_type_enum'
        ) THEN
    ...
    await queryRunner.query(
      `DROP TYPE IF EXISTS "public"."post_blocks_type_enum_old"`,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_posts_location_gist" ON "posts" USING GiST ("location") `,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_d00d52b628ccbca8200c30be69" ON "post_blocks" ("post_id", "layout_row", "layout_col") `,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_e067f8c9294df9c3ea668c055f" ON "post_media" ("block_id", "sort_order") `,
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "IDX_b0b5a4c05e3922d33923b39203" ON "post_contributors" ("post_id", "user_id") `,
    );
    await queryRunner.query(
      `DO $$ BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint WHERE conname = 'FK_74273612fc6076c533f12b9e6ba'
        ) THEN
