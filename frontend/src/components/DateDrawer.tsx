        ...
        <div className="w-full px-8 pt-8 pb-12 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-7 text-center">
            {['일', '월', '화', '수', '목', '금', '토'].map((day, idx) => (
              <div
                key={day}
                className={`py-4 text-[10px] font-bold ${idx === 0 ? 'text-rose-500' : 'text-gray-400'}`}
              >
                {day}
              </div>
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                ...
                >
                  ...
                  {mode === 'range' && isActive && (
                    <div
                      className={`absolute inset-y-1.5 bg-itta-point/10 transition-all
                  ...
                  <button
                    ...
                    className={`relative z-10 w-10 h-10 flex items-center justify-center text-xs font-bold transition-all
          ...
          <div className="flex flex-col gap-3">
            {mode === 'range' && (
              <button
                ...
                className="w-full py-4 rounded-2xl font-bold text-sm bg-gray-50 dark:bg-white/5 text-gray-500 dark:text-gray-400 active:scale-95 transition-all"
              >
                이 달 전체 선택
              </button>
            )}
