    ...
    <Drawer open={true} onOpenChange={(open) => !open && onClose()}>
      <DrawerContent className="h-[85%] bg-white dark:bg-[#121212]">
        <div className="w-full flex flex-col h-full overflow-hidden">
          {/* 헤더 */}
          <DrawerHeader className="px-4 sm:px-5 py-3 sm:py-4 flex flex-row items-center border-b border-gray-50 dark:border-white/5 space-y-0">
            {isManualInput && (
              <button
                onClick={() =>
              ...
              >
                <ArrowLeft className="w-5 sm:w-6 h-5 sm:h-6 text-itta-black dark:text-white" />
              </button>
            )}
            <DrawerTitle
              className={cn(
                'flex-1 text-center text-base sm:text-lg font-bold text-itta-black dark:text-white',
                isManualInput && 'mr-7 sm:mr-8',
              )}
            >
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar scrollbar-hide">
            {!isManualInput ? (
              <>
                ...
                <div className="p-3 sm:p-5">
                  <div className="relative">
                    ...
                    {query && (
                      <button
                        onClick={() => setQuery('')}
                        className="absolute right-3 sm:right-4 top-1/2 -translate-y-1/2 bg-gray-300 dark:bg-white/10 rounded-full p-0.5"
                      >
                        <X className="w-2.5 sm:w-3 h-2.5 sm:h-3 text-white dark:text-gray-400" />
                      </button>
                    )}
                  </div>
                ...
                {/* 결과 리스트 */}
                <div className="px-4 sm:px-5 pb-8 sm:pb-10">
                  {isLoading ? (
                    <div className="py-16 sm:py-20 flex flex-col items-center justify-center gap-2 sm:gap-3">
                      <Loader2 className="w-5 sm:w-6 h-5 sm:h-6 animate-spin text-[#10B981]" />
                      <span className="text-[11px] sm:text-xs text-gray-400">
                        검색 중 입니다 ...
                      </span>
                    </div>
                  ) : results.length > 0 ? (
                    <div className="divide-y space-y-3 sm:space-y-4 divide-gray-50 dark:divide-white/5">
                      {results.map((item) => (
                        <MediaField
                          key={item.externalId}
                          data={item}
                    ...
                    <div className="py-16 sm:py-20 flex flex-col items-center justify-center text-gray-400 gap-1.5 sm:gap-2">
                      <Search className="w-7 sm:w-8 h-7 sm:h-8 opacity-20" />
          ...
          {/* 푸터 영역 */}
          <div className="p-4 sm:p-6 bg-[#FBFBFB] dark:bg-[#1E1E1E] border-t border-gray-50 dark:border-white/5">
            {!isManualInput && (
              <div className="flex items-center gap-1.5 sm:gap-2 mb-4 sm:mb-6 px-0.5 sm:px-1">
                <AlertCircle className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-[#10B981]" />
                <p className="text-[11px] sm:text-[12px] font-bold text-itta-black dark:text-gray-300">
                  찾으시는 결과가 없나요?
                </p>
              </div>
            )}
            ...
            <button
              ...
              disabled={!isActive}
              className={cn(
                'w-full py-3 sm:py-4 rounded-xl sm:rounded-2xl text-xs sm:text-sm flex items-center justify-center gap-1.5 sm:gap-2 font-bold shadow-xl transition-all',
                isActive
