    ...
    <div className="px-4 sm:px-8 py-1.5 sm:py-2 space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <section className="space-y-3 sm:space-y-4">
        <div className="flex flex-col">
          <span className="text-[10px] sm:text-xs font-extrabold text-itta-point uppercase tracking-[0.2em] sm:tracking-widest mb-1">
            CATEGORY
          </span>
          <span className="text-[11px] sm:text-xs font-bold text-itta-gray3">
            카테고리
          </span>
        </div>
      ...
      <section className="space-y-1.5 sm:space-y-2">
        ...
        <div className="relative border-b border-gray-100 dark:border-white/10 py-2.5 sm:py-3">
          <input
            type="text"
            inputMode="numeric"
            value={manualYear}
          ...
          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex flex-col text-itta-gray3 gap-0.5 sm:gap-1">
            ...
            >
              <ChevronUp className="w-4 sm:w-5 h-4 sm:h-5 hover:text-itta-point" />
            </button>
            <button
              onClick={() => setManualYear(String(Number(manualYear) - 1))}
            >
              <ChevronDown className="w-4 sm:w-5 h-4 sm:h-5 hover:text-itta-point" />
            </button>
          </div>
        </div>
      </section>
