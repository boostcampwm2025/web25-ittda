...
export default function ViewOnMapButton({
  ...
  return (
    <div
      className={cn(
        'fixed bottom-20 left-1/2 -translate-x-1/2 z-40 animate-in slide-in-from-bottom-4 duration-500',
        className,
      )}
    >
      <button
      ...
      >
        <MapIcon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#10B981]" strokeWidth={2.5} />
        <span className="text-[11px] sm:text-xs font-bold tracking-tight">지도로 보기</span>
      </button>
    </div>
  );
}
