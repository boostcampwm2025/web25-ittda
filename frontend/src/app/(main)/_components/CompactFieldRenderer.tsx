...
export default function CompactFieldRenderer({
  ...
  switch (type) {
    ...
    case 'location':
      return (
        <div className="flex items-center gap-1.5 text-[11px] font-medium mb-2 dark:text-white/70 text-itta-black">
    ...
    case 'content':
      return (
        <p className="text-[13px] leading-relaxed font-normal mb-3 line-clamp-2 dark:text-gray-400 text-[#555555]">
          {data}
        </p>
      );
    case 'tags':
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {data.map((tag: string) => (
              ...
              >
                ...
                <span className="dark:text-gray-400 text-itta-black">
                  {tag}
                </span>
              </span>
            ))}
    ...
    case 'table':
      return (
        <div className="mb-3 overflow-hidden rounded-xl border border-black/5 dark:border-white/5 bg-white/50 dark:bg-transparent">
                ...
                (row: string[], i: number) => (
                  ...
                  >
                    {row.map((cell: string, j: number) => (
                      <td
                        key={j}
                        className="p-1.5 border-b border-r border-black/3 dark:border-white/2 last:border-r-0 truncate max-w-25 text-gray-500 dark:text-gray-400"
                      >
                        {cell}
                      </td>
                    ))}
    ...
    case 'media':
      return (
        <div className="flex items-center gap-3 p-3 rounded-xl border mb-3 transition-colors dark:bg-white/3 dark:border-white/5 bg-gray-50/50 border-gray-100">
          <Image
            src={data.image}
