...
export default function TagDashboard() {
  ...
  return (
    <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 shadow-xs border transition-colors duration-300 dark:bg-[#1E1E1E] dark:border-white/5 bg-white border-gray-100">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h4 className="text-xs sm:text-[13px] font-bold dark:text-white text-itta-black">
          태그
        </h4>
        <div className="p-0.5 sm:p-1 rounded-xl flex items-center dark:bg-black/20 bg-gray-50">
          <button
            onClick={() => setTagTab('recent')}
            className={cn(
              'cursor-pointer px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg text-[9px] sm:text-[10px] font-bold transition-all border',
              tagTab === 'recent'
      ...
      <div
        className={cn(
          ...
          currentTags.length > 0 ? 'mb-4 sm:mb-6' : 'mb-2',
        )}
      >
        {currentTags.length > 0 ? (
          currentTags.slice(0, 5).map((tag) => (
            <div
              key={tag.tag}
              className="flex items-center h-fit gap-1 px-2 py-1 sm:px-3 sm:py-1.5 border rounded-lg shadow-xs dark:bg-white/5 dark:border-white/5 bg-white border-gray-100"
            >
              <span className="text-[10px] sm:text-[11px] font-medium text-[#10B981]">
                #
              </span>
              <span className="text-[10px] sm:text-[11px] font-medium dark:text-gray-200 text-itta-black">
                {tag.tag}
              </span>
              <span className="text-[9px] sm:text-[10px] font-medium text-[#10B981]/90 ml-0.5">
                {tag.count}
              </span>
            </div>
          ))
      ...
      <div className="flex items-center gap-2 border-t pt-3 sm:pt-5 dark:border-white/5 border-gray-50">
        <Drawer>
          <DrawerTrigger className="cursor-pointer flex-1 flex items-center justify-center gap-1.5 sm:gap-2 py-2 sm:py-3 border rounded-xl text-[11px] sm:text-xs font-bold text-[#10B981] active:scale-95 transition-all dark:bg-white/5 dark:border-white/5 bg-white border-gray-100">
          ...
          <DrawerContent className="w-full px-6 sm:px-8 pt-4 pb-8 sm:pb-10">
            <DrawerHeader className="mx-0 px-0">
              <div className="flex justify-between items-center mb-6 sm:mb-8">
                ...
                <DrawerClose className="cursor-pointer p-1.5 sm:p-2 text-gray-400">
                  <X className="w-5 h-5 sm:w-6 sm:h-6" />
                </DrawerClose>
              </div>
            </DrawerHeader>

            <div className="max-h-[80vh] overflow-y-auto hide-scrollbar mb-6 sm:mb-8">
                ...
                {tags.all.map((tag) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'cursor-pointer flex items-center gap-0.5 px-3 py-2 sm:px-4 sm:py-2.5 border rounded-2xl transition-all active:scale-95',
                        isSelected
                          ? 'bg-itta-black dark:bg-itta-point border-itta-black text-white'
            ...
            <div className="flex gap-3 sm:gap-4">
              <button
                ...
                className="cursor-pointer flex-1 py-3 sm:py-4 rounded-2xl text-xs sm:text-sm font-bold dark:bg-white/5 dark:text-gray-500 bg-gray-50 text-gray-400"
              >
                초기화
              </button>
              <button
                ...
                className={cn(
                  'cursor-pointer flex-2 py-3 sm:py-4 rounded-2xl text-xs sm:text-sm font-bold transition-all flex items-center justify-center gap-1.5 sm:gap-2',
