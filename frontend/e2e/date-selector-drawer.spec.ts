...
test.describe('내 기록함 날짜 찾기 drawer', () => {
  ...
  test('날짜를 선택하면 해당 날짜의 기록을 타임라인으로 확인할 수 있다', async ({ page }) => {
    ...
    // 해당 날짜 타임라인 페이지로 이동
    await expect(page).toHaveURL(new RegExp(`/my/detail/${TODAY_STR}`), { timeout: 10000 });
  ...
  test('전월로 이동 후 월 기록 전체보기를 클릭하면 해당 월의 기록함으로 이동한다', async ({
    ...
    // 월 기록 전체보기 클릭
    await page.getByText('월 기록 전체보기').click();
    await expect(page).toHaveURL(new RegExp(`/my/month/${PREV_YEAR_MONTH}`), { timeout: 10000 });
  });

  test('년도 기록 전체보기를 클릭하면 해당 년도의 기록함으로 이동한다', async ({ page }) => {
    await openDateDrawer(page);

    await page.getByText(`${CURRENT_YEAR}년 기록 전체보기`).click();
    await expect(page).toHaveURL(new RegExp(`/my/year/${CURRENT_YEAR}`), { timeout: 10000 });
  });
});
