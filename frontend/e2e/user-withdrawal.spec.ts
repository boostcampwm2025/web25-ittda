 ...
 *
 * 3. 탈퇴 + Admin이 없는 그룹 → 그룹 삭제
 *    - 탈퇴자(A, fresh)만 Admin인 그룹은 A 탈퇴 시 그룹이 삭제된다
 *    - B(EDITOR, fresh)의 /shared에서 그룹이 사라진다
 *
...
test.describe('회원 탈퇴 - UI 흐름', () => {
  ...
  // TODO: 게스트 세션 탈퇴 후 세션 무효화 백엔드 구현 후 활성화
  // 현재 DELETE /api/me 후에도 게스트 세션 쿠키가 유효하여 /profile 접근이 차단되지 않음
  test.skip('탈퇴 API 호출 후 보호된 페이지 접근 시 /login으로 리다이렉트된다', async () => {
    if (!ctxARef) throw new Error('A context not initialized');
    const pageA = await ctxARef.newPage();
    try {
...
test.describe('회원 탈퇴 - 다른 Admin이 있으면 그룹 유지', () => {
  ...
  // TODO: Admin 탈퇴 후 edit/members 페이지 서버 에러 → 백엔드 버그 수정 후 활성화
  test.skip('A 탈퇴 후 B의 멤버 관리 페이지에는 B만 남아있다', async ({ page }) => {
    await page.goto(`/group/${groupId}/edit/members`);
    await expect(page.getByText(/멤버 관리 \(1\)/)).toBeVisible({ timeout: 8000 });
  });
});

// ── 시나리오 3: 탈퇴 + Admin 없는 그룹 → 그룹 삭제 ─────────────────────────
// TODO: 백엔드에서 마지막 Admin 탈퇴 시 그룹 자동 삭제 구현 후 활성화

test.describe.skip('회원 탈퇴 - 다른 Admin 없으면 그룹 삭제', () => {
  test.describe.configure({ mode: 'serial' });
  ...
  test('유일한 Admin A 탈퇴 후 그룹 페이지에 직접 접근하면 그룹이 없음을 확인할 수 있다', async () => {
    ...
    try {
      ...
      // 그룹이 없으므로 리다이렉트 또는 not-found 처리
